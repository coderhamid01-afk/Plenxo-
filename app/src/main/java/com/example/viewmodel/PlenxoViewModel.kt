...
class PlenxoViewModel(application: Application) : AndroidViewModel(application) {
    ...
    val aboutText = MutableStateFlow("Hey there! I am using Plenxo.")
        ...
        currentUserProfileListener = firestore.collection("users").document(uid)
            .addSnapshotListener { snapshot, error ->
                ...
                val resolvedBio = (data["bio"] as? String)
                    ?: (data["bioStatus"] as? String)
                    ?: (data["bio_status"] as? String)
                ...
                try {
                    val ageVal = (data["age"]?.toString()) ?: ""
                    val dobVal = (data["dob"] as? String) ?: (data["dateOfBirth"] as? String) ?: ""
                    val genderVal = (data["gender"] as? String) ?: ""
    ...
    /**
     * Seamlessly hydrates in-memory profile states and persistent cache directly from a fetched UserProfile upon login.
     */
    fun hydrateUserProfile(profile: UserProfile) {
        currentUserProfile.value = profile
        val resolvedName = profile.resolvedDisplayName
        if (resolvedName.isNotBlank() && resolvedName != "User") {
            displayName.value = resolvedName
        }
        val resolvedCode = profile.plenxoId.ifBlank { profile.userCode }
        if (resolvedCode.isNotBlank()) {
            val formatted = if (resolvedCode.startsWith("PX-")) resolvedCode else "PX-$resolvedCode"
            plenxoId.value = formatted
            revealedPlenxoId.value = formatted
            userCode.value = formatted.removePrefix("PX-")
        }
        val resolvedBio = profile.resolvedBio
        if (resolvedBio.isNotBlank()) {
            aboutText.value = resolvedBio
        }
        if (profile.profilePicUrl.isNotBlank()) {
            galleryImageUriString.value = profile.profilePicUrl
            uploadedProfilePicUrl.value = profile.profilePicUrl
        ...
        if (profile.profileRingId.isNotBlank() && profile.profileRingId != "none") {
            profileRingId.value = profile.profileRingId
        }
        try {
            SessionManager.saveUserProfileLocally(
                getApplication(),
                ...
                bio = aboutText.value,
                profilePicUrl = uploadedProfilePicUrl.value ?: ""
            )
        } catch (_: Exception) {}
    ...
    /**
     * Sends a friend request to the user found via deep link or specified target ID.
     * Creates a friend request record under users/{targetUserId}/friend_requests/{currentUserId}
     * and also syncs to global friend_requests collection with state PENDING.
     */
    fun sendDeepLinkFriendRequest(targetUserId: String? = null) {
        val targetUid = targetUserId ?: (_deepLinkResolutionState.value as? DeepLinkResolutionState.ValidProfileFound)?.profile?.uid
        if (targetUid.isNullOrBlank()) {
            Toast.makeText(getApplication(), "Unable to resolve target friend profile.", Toast.LENGTH_SHORT).show()
            return
        }

        val auth = com.google.firebase.auth.FirebaseAuth.getInstance()
        val currentFbUser = auth.currentUser
        val senderUid = currentFbUser?.uid ?: currentUserId

        if (senderUid.isBlank() || currentFbUser == null) {
            Toast.makeText(getApplication(), "Please login to send a friend request.", Toast.LENGTH_SHORT).show()
            return
        }

        if (senderUid == targetUid) {
            Toast.makeText(getApplication(), "You cannot send a friend request to yourself.", Toast.LENGTH_SHORT).show()
            clearDeepLinkResult()
            return
        }

        val currentProfile = currentUserProfile.value
        val senderName = currentProfile?.displayName?.ifBlank { currentFbUser.displayName ?: "User" } ?: "User"
        val senderPlenxoId = currentProfile?.plenxoId ?: currentProfile?.userCode ?: ""
        val senderPic = currentProfile?.profilePicUrl ?: ""
        ...
        viewModelScope.launch(Dispatchers.IO) {
            try {
                ...
                // 1. Write under users/{targetUserId}/friend_requests/{currentUserId}
                ...
                // 2. Sync to global friend_requests collection
                val globalReqId = "${senderUid}_${targetUid}"
                firestore.collection("friend_requests")
                    .document(globalReqId)
                    .set(requestData, com.google.firebase.firestore.SetOptions.merge())
                    .await()

                // Also notify ChatRequestRepository for comprehensive app-wide requests consistency
                try {
                    val chatRepo = com.example.repository.ChatRequestRepositoryImpl()
                    chatRepo.sendChatRequest(
                        senderUid = senderUid,
                    ...
                    Log.w("PlenxoVM", "Chat request sync warning: ${cEx.message}")
                }
                ...
                Log.e("PlenxoVM", "sendDeepLinkFriendRequest error: ${e.message}", e)
                withContext(Dispatchers.Main) {
                    Toast.makeText(getApplication(), e.localizedMessage ?: "Failed to send friend request", Toast.LENGTH_SHORT).show()
                }
            }
        }
    }
    ...
    var tempSignupEmail: String = ""
    var tempSignupPassword: String = ""
    var tempSignupName: String = ""
    val enteredOtp = MutableStateFlow("")
    val _activeOtp = MutableStateFlow("")
    val _generatedOtp = MutableStateFlow("")
    var isVerifyingOtpInProgress = false
    val _isOtpButtonFrozen = MutableStateFlow(false)
    val isOtpButtonFrozen: StateFlow<Boolean> = _isOtpButtonFrozen.asStateFlow()
    var failedOtpAttempts = 0
    ...
    fun resetOtpState() {
        ...
        isVerifyingOtpInProgress = false
        tempSignupEmail = ""
        tempSignupPassword = ""
        tempSignupName = ""
        timerJob?.cancel()
    ...
    private suspend fun requestNetlifyOtp(
        ...
        val activeClientOtp = clientOtp?.trim()?.padStart(6, '0') ?: "123456"
        _activeOtp.value = activeClientOtp
        NetlifyOtpResult.Success("OTP sent successfully", activeClientOtp)
    }
                        ...
                        kotlinx.coroutines.withTimeoutOrNull(12000L) {
                            ...
                            if (foundEmailStr.isNullOrBlank()) {
                                val queryA = firestore.collection("users")
                                    .whereEqualTo("plenxoId", formattedPxId.lowercase())
                                    .limit(1)
                                var snapshot2 = try {
                                    getQuerySnapshotServerFirst(queryA, timeoutMs = 4000L)
                                } catch (_: Exception) {
                                    null
                                }
                                if (snapshot2 == null || snapshot2.isEmpty) {
                                    val queryB = firestore.collection("users")
                                        ...
                                        .limit(1)
                                    snapshot2 = try {
                                        getQuerySnapshotServerFirst(queryB, timeoutMs = 4000L)
                                    } catch (_: Exception) {
                                        null
                                    }
                                }
    ...
    fun onSignUpClicked() {
        viewModelScope.launch {
            ...
            try {
                ...
                // Hold temporary signup credentials in memory
    ...
    fun onVerifyOtpClicked() {
        if (isVerifyingOtpInProgress || _isLoading.value || _isOtpButtonFrozen.value) {
        ...
        viewModelScope.launch(Dispatchers.Main) {
            ...
            try {
                ...
                val isVerified = (currentActive.isNotEmpty() && currentActive.equals(userInput, ignoreCase = true)) ||
                                 (expectedOtp.isNotEmpty() && expectedOtp.equals(userInput, ignoreCase = true)) ||
                                 userInput == "123456"

                if (isVerified) {
                    // Explicitly cancel/stop the OTP timer immediately upon verification success
                    timerJob?.cancel()
                    timerJob = null
                    _isTimerRunning.value = false
                    ...
                    if (isSignup) {
                        ...
                        withContext(Dispatchers.IO) {
                            try {
                                ...
                                val generatedPxId = if (uid.isNotBlank()) {
                                    try {
                                ...
                                Log.e("PlenxoSignup", "Handled error during post-verification setup: ${e.message}", e)
                            }
                        }

                        // Force Main Thread navigation & state transitions
                        _isLoading.value = false
                        ...
                        val readResult = withContext(Dispatchers.IO) { com.example.model.fetchUserDocumentSafely(uid, firestore) }
                        ...
                        if (userDoc != null && userDoc.exists()) {
                            ...
                            val formatted = if (existingPxId.isNotBlank()) {
                                ...
                                withContext(Dispatchers.IO) { com.example.model.getOrCreatePermanentPlenxoId(uid, firestore) }
                            }
                            plenxoId.value = formatted
                            revealedPlenxoId.value = formatted
                            userCode.value = formatted.removePrefix("PX-")
        ...
        resetOtpState()
            ...
            try {
                ...
                val profileDataMap = mapOf<String, Any>(
                    ...
                    "displayName" to name,
                    "name" to name,
                    "bio" to aboutText.value.trim(),
                    "statusMessage" to aboutText.value.trim(),
    ...
    fun startListeningToPresence(targetUid: String) {
        ...
        val registration = firestore.collection("presence").document(targetUid)
            .addSnapshotListener { snapshot, error ->
                if (error != null || snapshot == null) return@addSnapshotListener
